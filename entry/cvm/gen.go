...
typedef struct CrossRequestRef {
  struct ListRef env;
  struct ListRef data;
} CrossRequestRef;
...
func CTEEServer_start(req C.CrossRequestRef, slot *C.void, cb *C.void) {
...
func newCrossRequest(p C.CrossRequestRef) CrossRequest {
	return CrossRequest{
		env:  new_list_mapper_primitive(newC_uint8_t)(p.env),
		data: new_list_mapper_primitive(newC_uint8_t)(p.data),
	}
}
func ownCrossRequest(p C.CrossRequestRef) CrossRequest {
	return CrossRequest{
		env:  new_list_mapper(newC_uint8_t)(p.env),
		data: new_list_mapper(newC_uint8_t)(p.data),
	}
}
func cntCrossRequest(s *CrossRequest, cnt *uint) [0]C.CrossRequestRef {
	_ = s
	_ = cnt
	return [0]C.CrossRequestRef{}
}
func refCrossRequest(p *CrossRequest, buffer *[]byte) C.CrossRequestRef {
	return C.CrossRequestRef{
		env:  ref_list_mapper_primitive(refC_uint8_t)(&p.env, buffer),
